...
const { NotFoundError, InvariantError } = require('../../commons/exceptions');
...
class AlbumsHandler {
  ...
  async postAlbumCoverByIdHandler(request, h) {
    ...
    const { cover } = this._validator.validatePutAlbumCoverPayload(
      request.payload,
    );
    const fileExtension = cover.hapi.filename.split('.').pop();

    const filename = await this._storageService.saveAlbumArt(
      albumId,
      cover,
      fileExtension,
    );
  ...
  async postLikeAlbumByIdHandler(request, h) {
    ...
    const isLiked = await this._userAlbumLikesService.isUserAlreadyLikedAlbum(
      userId,
      albumId,
    );
